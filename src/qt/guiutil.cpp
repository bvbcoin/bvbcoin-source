...
{
    // Convert bvbcoin:// to bvbcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bvbcoin://"))
    {
        uri.replace(0, 12, "bvbcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BVBCoin.lnk";
}
...
{
    return GetAutostartDir() / "bvbcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BVBCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bvbcoin-qt [" + tr("command-line options") + "]                     " + "\n";
